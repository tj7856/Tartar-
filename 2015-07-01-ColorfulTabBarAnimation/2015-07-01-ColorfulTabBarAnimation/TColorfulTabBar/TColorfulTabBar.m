...
//        UIView *view = [[UIView alloc] initWithFrame:CGRectMake(itemWidth * i, 0, itemWidth, SELF_HEIGHT)];
        LYTtarBTN *view = [[LYTtarBTN alloc] initWithFrame:CGRectMake(itemWidth * i+itemWidth/2, SELF_HEIGHT/2, 4, 4)];
...
- (void)tabBar:(UITabBar *)tabBar didSelectItem:(UITabBarItem *)item {
    ...
    self.toIndex = index;
//    [self animation];
    for (int i=0; i<self.colorfulView.subviews.count; i++) {
        if (i==index) {
            self.colorfulView.subviews[i].hidden = NO;
            [((LYTtarBTN *)(self.colorfulView.subviews[index])) doAnimation ];
        }
        else
        {
            self.colorfulView.subviews[i].hidden = YES;
        }
